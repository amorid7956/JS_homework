...
// №1
    showScore()
    {
        ...
        score.innerText = `Текущий счёт равен: ${this.body.length - 1}`;
    },
    //throughObstacles
//////////////////////////////
//////////////////////////////
    ...
    getNextStepHeadPoint() {
        ...
        switch(this.direction) {
            case 'up':
                ...
                {
                    obstaclePoint = firstPoint.y - 1;
                }
                return {x: firstPoint.x, y: obstaclePoint};
            case 'right':
                if (firstPoint.x + 1 === 21) 
                {
                    obstaclePoint = 0;
                }
                else
                {
                    obstaclePoint = firstPoint.x + 1;
                }
                return {x: obstaclePoint, y: firstPoint.y};
            case 'down':
                if (firstPoint.y + 1 === 21) 
                {
                    obstaclePoint = 0;
                }
                else
...
const game = {
    ...
    canMakeStep() {
        ...
        return !this.snake.isOnPoint(nextHeadPoint) && !this.obstacle.isOnPoint(nextHeadPoint);
/*            nextHeadPoint.x < this.config.getColsCount() &&
            nextHeadPoint.y < this.config.getRowsCount() &&
            nextHeadPoint.x >= 0 && !this.obstacle.isOnPoint(nextHeadPoint) && //Дополнительная проверка
            nextHeadPoint.y >= 0;*/
    },
    ...
    getRandomFreeCoordinates() {
        const exclude = [this.obstacle.getCoordinates(), this.food.getCoordinates(), ...this.snake.getBody()];
